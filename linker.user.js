...
`
    ...
    const imdbPageUtils = (() => {
        function createLetterboxdElement(imdbId) {
            ...
            letterboxdElement.id = "linker-letterboxd-link"
    ...
    async function imdbPageInjector() {
        ...
        if (imdbId) {
            const parentElement = commonUtils.element.parent()
            const letterboxdElement = imdbPageUtils.element.letterboxd(imdbId)
            const dividerElement = commonUtils.element.divider()
            const loadingElement = commonUtils.element.loading()

            commonUtils.waitForElement("div:has( > div[data-testid='hero-rating-bar__user-rating'])", 10000, isMobile ? 2 : 1).then((location) => {
            ...
            if (tmdbData) {
                const imdbElement = imdbPageUtils.element.tmdbButton(tmdbData)
    ...
    async function imdbPersonPageInjector() {
        ...
        if (imdbId) {
            ...
            commonUtils.waitForElement("div:has( > .starmeter-logo)", 10000, isMobile ? 2 : 1).then((location) => {
                location.insertBefore(parentElement, location.firstChild)
                parentElement.appendChild(loadingElement)
            })
    ...
    async function tmdbTitlePageInjector() {
        ...
        if (tmdbId) {
            const parentElement = commonUtils.element.parent()
            const letterboxdElement = tmdbTitlePageUtils.element.letterboxd(tmdbId, path[1])
            const divider = commonUtils.element.divider()
            const imdbContainer = tmdbTitlePageUtils.element.imdbContainer()
            const loadingElement = commonUtils.element.loading()

            commonUtils.waitForElement(`.header.poster${isMobile ? " > .title" : ""}`, 10000).then((location) => {
                if (isMobile) {
            ...
            // inject imdb link
            const imdbLink = tmdbTitlePageUtils.element.imdbLink(imdbId, commonUtils.svg.ImdbSvg)
            imdbContainer.insertBefore(imdbLink, loadingElement)

            // inject imdb rating
            const [imdbRating, imdbNumRating] = await commonUtils.getImdbRating(imdbId)
            const imdbRatingElement = tmdbTitlePageUtils.element.imdbRating(imdbRating, imdbNumRating)
    ...
    const tmdbPersonPageUtils = (() => {
        function createImdbLinkElement(imdbId) {
            ...
            childDiv.innerHTML = commonUtils.svg.ImdbSvgWithoutBg
    ...
    async function tmdbPersonPageInjector() {
        ...
        if (!imdbId) {
            parentElement.removeChild(loadingElement)
            const socialLinksElement = await commonUtils.waitForElement(`.social_links`, 10000)
            socialLinksElement.removeChild(parentElement)
            return
        }

        const imdbLinkElement = tmdbPersonPageUtils.element.imdbLink(imdbId)
        parentElement.removeChild(loadingElement)
        parentElement.appendChild(imdbLinkElement)
    }

    function letterboxdPageInjector() {
